...
const MAPA_CENTRO = (typeof MARCA !== 'undefined' && MARCA.MAPA_CENTRO) || { lat: 4.289013, lng: -74.814098 };

/* Sonidos: los MISMOS de JHONNY ASISTENCIA (capa-3 los precarga y los
   reproduce sin retardo). */
const SOUNDS = {
  /* 30/07/2026 · en la carpeta img/ de este repo, no en un hosting externo.
     Se piden por fetch/Web Audio y desde otro dominio eso arrastra CORS
     (capa-3-sonido.js ya lo tenía documentado); en el MISMO origen el
     problema desaparece. Ruta relativa: los sirve el mismo GitHub Pages
     que sirve la app. */
  ok:   'img/ok.mp3',
  info: 'img/info.mp3',
  err:  'img/error.mp3'
...
/* ============================================================
   ...
   ============================================================ */
const SES_KEY  = nsKey('jpComSesion');
const DISP_KEY = nsKey('jpComDisp');
let SES = null;
...
window.addEventListener('beforeinstallprompt', e => { e.preventDefault(); deferredPrompt = e; updateInstallSection(false); });
...
function render() {
  ...
  if (base === 'instalar' && isStandalone()) { history.replaceState(null, '', location.pathname + location.search + '#/'); return render(); }
  if (!isStandalone() && !sessionStorage.getItem(nsKey('continuedWeb')) && base !== 'instalar') return viewInstalar();

  sesLeer();
  const privadas = ['home', 'politicas', 'actualizar', 'validar', 'clientes', 'preview', 'avisos', 'resenas'];
  ...
  switch (base) {
    ...
    case 'resenas':    return viewResenas();      // 03/08/2026
  ...
  montar(`
    <div class="login-wrap"><div class="login-card">
      ...
      <div id="install-box" style="margin-top:16px;">
        ...
        <button id="btn-cont-web" class="btn btn-ghost btn-block" style="margin-top:10px;">🌐 Seguir en el navegador</button>
      </div>
      ${footBrand()}
    </div></div>`);
...
function pedirDueno() {
  $('#rg-dueno').innerHTML = `
    <div class="aviso aviso-info" style="margin-top:10px;">
      ...
      <p class="small">Ese número no está en la base de la app. Escribe la <b>cédula del dueño</b>: debe estar registrada en la app de ${esc(MV.candidato)}.</p>
...
async function montarMapa(idMapa, idTxt, idHere, idOk, idNo, onSet, valorActual) {
  ...
  const inicio = coordsDe(valorActual) || MAPA_CENTRO;
...
function pintarHome() {
  ...
  $('#hm-body').innerHTML = `
    ...
    <div class="tools">
      ...
      <button class="tool" id="hm-res"><span>⭐</span>Estrellas y reseñas</button>
...
async function viewPoliticas() {
  ...
  try {
    ...
    $('#po-body').innerHTML = `
      <div class="hoja">
        <div class="hoja-top"><img src="${APP_ICON}" alt="" /><div><b>${esc(r.titulo)}</b><span>Comercios aliados · ${esc(MV.lema)}</span></div></div>
...
async function procesarLectura(raw) {
  ...
  if (!p) { sonar('err'); vibrar(200); resultado('err', 'QR no reconocido', `Ese código no es de la app de ${MV.candidato}.`); return; }
  await validar(p);
}
...
/* ============================================================
   ESTRELLAS Y RESEÑAS (03/08/2026)
   Lo que la gente dice del comercio desde la app pública. El
   comerciante puede responder, dar me gusta y ocultar una reseña.
   Sin avisos: esto se ve cuando él entra, no le llega nada.
   ============================================================ */
let RS = { resumen: null, items: [], respondiendo: '' };
...
function rsIniciales(n) {
  const p = String(n || '').trim().split(/\s+/);
  return ((p[0] || ' ')[0] + ((p[1] || '')[0] || '')).toUpperCase() || 'US';
}

function rsAvatar(it) {
  const f = String(it.foto || '').trim();
  const s = f.toLowerCase();
  const esDefault = !f || ['user_zefosv', 'marca/user.png', 'data:image/svg+xml'].some(x => s.indexOf(x) !== -1);
  return esDefault
    ? `<div class="rs-av">${esc(rsIniciales(it.nombre))}</div>`
    : `<div class="rs-av has-photo"><img src="${esc(f)}" alt="" onerror="this.parentNode.classList.remove('has-photo');this.replaceWith(document.createTextNode('${esc(rsIniciales(it.nombre))}'))"></div>`;
}

function rsEstrellas(n) {
  let s = '';
  for (let i = 1; i <= 5; i++) s += `<span class="rs-stf ${i <= n ? 'on' : ''}">★</span>`;
  return `<span class="rs-stfs">${s}</span>`;
}

async function viewResenas() {
  montar(`${backbar('Estrellas y reseñas', 'home')}<div class="pad stack" id="rs-body">${loadingBox('Buscando lo que dice la gente…')}</div>`);
  try {
    const r = await api('com.social', auth());
    if (!r.ok) throw new Error(r.msg || 'sin datos');
    RS = { resumen: r.resumen, items: r.items || [], tope: r.tope || 600, respondiendo: '' };
    rsPintar();
  } catch (e) {
    if (/sesión/i.test(String(e.message))) { sesBorrar(); return go('login'); }
    $('#rs-body').innerHTML = `<div class="ev-card"><p class="muted">No se pudieron cargar las reseñas.</p>
      <button class="btn btn-ghost btn-block" id="rs-retry" style="margin-top:10px;">Reintentar</button></div>`;
    $('#rs-retry').onclick = () => viewResenas();
  }
}

function rsPintar() {
  const res = RS.resumen || { estrellas: 0, promedio: 0, dist: [0, 0, 0, 0, 0], resenas: 0, ocultas: 0 };
  ...
  const sinResponder = RS.items.filter(x => !x.respuesta && !x.oculta).length;

  const tarjetas = RS.items.map(it => `
    <article class="rs-item ${it.oculta ? 'off' : ''}" data-id="${esc(it.id)}">
      <div class="rs-item-top">
        ${rsAvatar(it)}
        <div class="rs-item-id">
          <b>${esc(it.nombre)}</b>
          <span class="small muted">${rsFechaCorta(it.fecha)}${it.editado ? ' · editada' : ''}${it.oculta ? ' · oculta' + (it.ocultaPor === 'ADMIN' ? ' por el administrador' : '') : ''}</span>
        </div>
        ${it.estrellas ? rsEstrellas(it.estrellas) : ''}
      </div>
      <p class="rs-item-tx">${esc(it.texto)}</p>
      ${it.respuesta ? `<div class="rs-reply"><b>Tu respuesta</b><p>${esc(it.respuesta)}</p><span class="small muted">${rsFechaCorta(it.respuestaFecha)}</span></div>` : ''}
      ${RS.respondiendo === it.id ? `
        <div class="rs-write">
          <textarea id="rs-txt" maxlength="${RS.tope}" rows="3" placeholder="Responde con amabilidad…">${esc(it.respuesta || '')}</textarea>
          <div class="rs-write-foot">
            <span class="small muted" id="rs-count">0/${RS.tope}</span>
            <div class="rs-write-btns">
              <button class="btn btn-quiet" data-cancel>Cancelar</button>
              ${it.respuesta ? `<button class="btn btn-quiet" data-borrar="${esc(it.id)}">Borrar respuesta</button>` : ''}
              <button class="btn btn-primary" data-enviar="${esc(it.id)}">Enviar</button>
            </div>
          </div>
        </div>` : ''}
      <div class="rs-item-acts">
        <button class="rs-like ${it.miLike ? 'on' : ''}" data-like="${esc(it.id)}">★ <b>${it.likes}</b></button>
        <button class="rs-mini" data-responder="${esc(it.id)}">${it.respuesta ? 'Editar respuesta' : 'Responder'}</button>
        <button class="rs-mini ${it.oculta ? '' : 'danger'}" data-ocultar="${esc(it.id)}" data-val="${it.oculta ? '0' : '1'}"
          ${it.oculta && it.ocultaPor === 'ADMIN' ? 'disabled title="La ocultó el administrador"' : ''}>${it.oculta ? 'Mostrar' : 'Ocultar'}</button>
      </div>
    </article>`).join('');

  $('#rs-body').innerHTML = `
    <div class="ev-card rs-resumen">
      <div class="rs-prom">
        <b>${res.estrellas ? String(res.promedio).replace('.', ',') : '—'}</b>
        ${rsEstrellas(Math.round(res.promedio))}
        <span class="small muted">${res.estrellas} calificación${res.estrellas === 1 ? '' : 'es'}</span>
      </div>
      <div class="rs-bars">${barras}</div>
    </div>

    <div class="rs-kpis">
      <div class="rs-kpi"><b>${res.resenas}</b><span>reseñas visibles</span></div>
      <div class="rs-kpi ${sinResponder ? 'warn' : ''}"><b>${sinResponder}</b><span>sin responder</span></div>
      <div class="rs-kpi"><b>${res.ocultas}</b><span>ocultas</span></div>
    </div>

    <div class="rs-list">${tarjetas || `<div class="ev-card"><p class="muted">Todavía nadie ha dejado una reseña. Cuando lo hagan, aparecerán aquí.</p></div>`}</div>
    ${footBrand()}`;

  const txt = $('#rs-txt');
  if (txt) {
    const cnt = $('#rs-count');
    const pinta = () => { cnt.textContent = `${txt.value.length}/${RS.tope}`; };
    txt.oninput = pinta; pinta(); txt.focus();
  }
  $$('#rs-body [data-responder]').forEach(b => b.onclick = () => { RS.respondiendo = b.dataset.responder; rsPintar(); });
  const cancel = $('#rs-body [data-cancel]');
  if (cancel) cancel.onclick = () => { RS.respondiendo = ''; rsPintar(); };
  const enviar = $('#rs-body [data-enviar]');
  if (enviar) enviar.onclick = () => rsResponder(enviar.dataset.enviar, $('#rs-txt').value, enviar);
  const borrar = $('#rs-body [data-borrar]');
  if (borrar) borrar.onclick = () => rsResponder(borrar.dataset.borrar, '', borrar);
  $$('#rs-body [data-like]').forEach(b => b.onclick = () => rsLike(b.dataset.like, b));
  $$('#rs-body [data-ocultar]').forEach(b => b.onclick = () => rsOcultar(b.dataset.ocultar, b.dataset.val, b));
}

async function rsResponder(id, texto, btn) {
  saving(btn, true);
  try {
    const r = await api('com.resenaResponder', {}, 'POST', Object.assign({ id: id, texto: texto }, auth()));
    RS.items = r.items || RS.items;
    RS.resumen = r.resumen || RS.resumen;
    RS.respondiendo = '';
    rsPintar();
    toast(texto ? 'Respuesta publicada' : 'Respuesta borrada');
  } catch (e) {
    saving(btn, false);
    toast(e.message || 'No se pudo responder', 'err');
  }
}
...
async function rsLike(id, btn) {
  btn.disabled = true;
  try {
    const r = await api('com.resenaLike', Object.assign({ id: id }, auth()));
    const it = RS.items.find(x => x.id === id);
    if (it) { it.likes = r.likes; it.miLike = r.miLike; }
    rsPintar();
  } catch (e) {
    btn.disabled = false;
    toast(e.message || 'No se pudo registrar el me gusta', 'err');
  }
}

async function rsOcultar(id, val, btn) {
  saving(btn, true);
  try {
    const r = await api('com.resenaOcultar', {}, 'POST', Object.assign({ id: id, ocultar: val }, auth()));
    RS.items = r.items || RS.items;
    RS.resumen = r.resumen || RS.resumen;
    rsPintar();
    toast(val === '1' ? 'La reseña ya no se ve en la app pública' : 'La reseña vuelve a verse');
  } catch (e) {
    saving(btn, false);
