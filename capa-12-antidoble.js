/* ============================================================================
 * CAPA 12 · ANTI DOBLE CLIC  (app COMERCIOS)   ·   31/07/2026
 * ----------------------------------------------------------------------------
 * Misma capa que ya está publicada en la app pública. Aquí va ajustada a este
 * repo: incluye el cursor de espera en el teclado del PIN y en las pestañas del login,
 *          que hoy no tienen ninguna regla :disabled en el CSS.
 *
...
(function () {
  ...
  /* ---------------------------------------------------------------------
     ...
     --------------------------------------------------------------------- */
  try {
    var ESPERA = ['.btn', 'button', '.iconbtn', '.login-tab', '.pin-key', '.espec', '.hero-play', '.img-drop', '.tool', '.combo-btn', '.combo-item', '.combo-fil', '.chk', '.check', '.ev-row', '.ev-chip'];
    var reglas = '';
  ...
  /* --- parche del asignador `onclick` -------------------------------------
     ...
     (62 en app.js): quedan cubiertas de una. No usa addEventListener de clic. */
  var objetivos = [];
